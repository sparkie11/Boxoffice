...
interface TableSectionProps {
  ticketHistoryData: TicketHistoryResponse | null;

}


const TableSectionDynamic: React.FC<TableSectionProps> = ({
  ticketHistoryData,
}) => {
  ...
  return (
    <section className="bg-white mt-1 mb-20 rounded-lg shadow-md">
      <div className="p-4">
        ...
        {ticketHistoryData && ticketHistoryData.data.data.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {ticketHistoryData.data.data.map((item, index) => (
                  <tr key={index}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.match_info.match_name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.match_info.match_date}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.match_info.stadium_name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.tickets[0]?.ticket_type || 'N/A'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.tickets[0]?.ticket_category || 'N/A'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.tickets[0]?.quantity || 'N/A'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.tickets[0]?.price_type} {item.tickets[0]?.price}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <p className="text-gray-500">No ticket history data available.</p>
        )}
      </div>
      <div
              ...
              <tr>
                ...
                <th className=" px-4 py-2 sticky left-0 top-0 bg-gray-50 z-20 border-r border-gray-200">
                  <input
                    ...
                    checked={
                      selectedItem?.length === inventory.length &&
                      inventory.length > 0
                    }
